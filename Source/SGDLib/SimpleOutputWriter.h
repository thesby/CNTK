...
public:
    ...
    void WriteMatrix(FILE* f, const Matrix<ElemType> &outputValues, std::wstring nodeName, ElemType* tempArray, MBLayoutPtr pMBLayout, 
    ...
    void WriteOutput(IDataReader& dataReader, size_t mbSize, std::wstring outputPath, const std::vector<std::wstring>& outputNodeNames, const WriteFormattingOptions & formattingOptions, size_t numOutputSamples = requestDataSize, bool nodeUnitTest = false)
    {
        ...
        while (DataReaderHelpers::GetMinibatchIntoNetwork<ElemType>(dataReader, m_net, nullptr, false, false, inputMatrices, actualMBSize))
        {
            ...
            for (auto & onode : outputNodes)
            {
                ...
                WriteMatrix(f, outputValues, onode->NodeName(), tempArray, onode->GetMBLayout(), formattingOptions, formatChar, valueFormatString, labelMapping, numMBsRun);
            } // end loop over nodes
            ...
            if (nodeUnitTest)
            {
                for (auto & inodeFile : outputStreamsForGradients)
                {
                    ...
                    WriteMatrix(*file, gradient, inode->NodeName() + L"__Gradient", tempArray, inode->GetMBLayout(), formattingOptions, formatChar, valueFormatString, labelMapping, numMBsRun);
                }
            }
            totalEpochSamples += actualMBSize;
